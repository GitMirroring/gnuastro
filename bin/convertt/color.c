...
static void
color_from_mono_blank(struct converttparams *p, float *bc,
                      float *r, float *g, float *b)
{
  /* On first occasion the 'bc' array is all NaN, so find the color's
     values. */
  if(isnan(bc[0]))
    gal_color_in_rgb(gal_color_name_to_id(p->cmapblankcolor),
                     bc);
...
static void
color_from_mono_core_hsv(float *f, float min, float max,
                         float h_min, float h_max,
                         float *r, float *g, float *b)
{
  int i;
  float h, s=1, v, hh, ff, P, q, t;
...
static void
color_from_mono_core_viridis(float *f, float min, float max, float *r,
                             float *g, float *b)
{
                         ...
                         float min, float max, float p1, float p2)
{
  ...
  /* Write the color map.*/
  fprintf(cmapfp, "  /pgfplots/colormap={gnuastro%s}{\n", name);
  for(c=min; c<=max; c+=(max-min)/255)
    {
      switch(p->colormap->status)
        {
        case COLOR_HSV:
          color_from_mono_core_hsv(&c, min, max, p1, p2, &r, &g, &b);
          break;
        case COLOR_VIRIDIS:
          color_from_mono_core_viridis(&c, min, max, &r, &g, &b);
          break;
        case COLOR_GRAY:
        case COLOR_SLS:
      ...
      fprintf(cmapfp, "    rgb=(%.2f,%.2f,%.2f)\n", r, g, b);
    }
  if(cmapfp) fprintf(cmapfp, "  }\n");
}
...
void
color_from_mono_hsv(struct converttparams *p, FILE *cmapfp)
{
  float bc[3]={NAN, NAN, NAN};
  gal_data_t *R, *G, *B, *channel;
  float *r, *g, *b, *f, *fp, min, max;
  float *params=p->colormap->next->array;
  float h_min=params[0], h_max=params[1];
  ...
  do
    {
      /* Set the RGB value (between 0 and 1) for this pixel. */
      if(isnan(*f)) color_from_mono_blank(p, bc, r, g, b);
      else color_from_mono_core_hsv(f, min, max, h_min, h_max, r, g, b);

      /* Convert the RGB values to 0 and 255 for color formats. */
      *r++ *= 255;
      *g++ *= 255;
      *b++ *= 255;
    }
  ...
  if(cmapfp)
    color_from_mono_colorbar(p, cmapfp, min, max, h_min, h_max);
...
/* From SAO DS9: */
void
color_from_mono_sls(struct converttparams *p, FILE *cmapfp)
{
  float bc[3]={NAN, NAN, NAN};
  gal_data_t *R, *G, *B, *channel;
  float *r, *g, *b, *f, *fp, min, max;
  ...
  do
    {
      if(isnan(*f)) color_from_mono_blank(p, bc, r, g, b);
      else color_from_mono_core_sls(p, f, min, max, r, g, b);
...
void
color_from_mono_viridis(struct converttparams *p, FILE *cmapfp)
{
  float bc[3]={NAN, NAN, NAN};
  gal_data_t *R, *G, *B, *channel;
  float *r, *g, *b, *f, *fp, min, max;
  ...
  do
    {
      if(isnan(*f)) color_from_mono_blank(p, bc, r, g, b);
      else color_from_mono_core_viridis(f, min, max, r, g, b);
...
color_map_prepare(struct converttparams *p)
{
  ...
  /* In case a colormap should be written to file. */
  if(p->cmappgfplotsfile)
    {
      errno=0;
      cmapfp=fopen(p->cmappgfplotsfile, "w");
      if(cmapfp==NULL)
        error(EXIT_FAILURE, errno, "%s: couldn't open to write "
              "colormap", p->cmappgfplotsfile);
      fprintf(cmapfp, "\\pgfplotsset{\n");
    }
  ...
  switch(p->colormap->status)
    ...
    /* Colormaps are inside these functions. */
    ...
    case COLOR_GRAY:
      ...
      if(cmapfp)
        fprintf(cmapfp, "  /pgfplots/colormap={gnuastrogray}{\n"
                "    rgb255=(0,0,0)\n"
                "    rgb255=(255,255,255)\n  }\n");
      break;
  ...
  if(p->cmappgfplotsfile)
    {
      ...
      if(fclose(cmapfp))
        error(EXIT_FAILURE, errno, "%s: couldn't close file after "
              "writing colormap", p->cmappgfplotsfile);
      if(!p->cp.quiet) printf("Colormap: '%s'\n", p->cmappgfplotsfile);
    }
}
